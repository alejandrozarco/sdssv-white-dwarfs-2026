...
| file | content | script |
|---|---|---|
| `tables/magnetic_zeeman.csv` | 28 white dwarfs: H-alpha and H-beta outer-component separations | `zeeman_split.py` |
...
## Methods
- **SDSS-V spectra.** mwmStar coadds and mwmVisit spectra from `data.sdss.org/sas/dr20/spectro/astra/0.8.1`. For visits with `in_stack = True`, the XCSAO velocity shift is removed (wavelength = grid × (1 + v_xcsao/c)); other visits are used as delivered. Coadds of visits are inverse-variance weighted on a common log-wavelength grid (`sdssv.py`).
- **Zeeman splitting.** H-alpha (6300-6830 Å) and H-beta (4660-5060 Å) are each fitted with:
  ...
  B_split is the half-separation of the outer components divided by 4.67×10⁻¹³ λ0² (20.13 Å/MG at H-alpha, 11.04 Å/MG at H-beta). This is a linear-Zeeman scale, not a model-atmosphere field.
